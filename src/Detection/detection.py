...
device = torch.device(
    "cuda:0" if torch.cuda.is_available() else "cpu")  ##Assigning the Device which will do the calculation
...
def testPrediction(directory):
    countUW = 0
    countCW = 0
    parent = parent_src.parent
    fileCount = len([entry for entry in os.listdir(directory) if os.path.isfile(os.path.join(directory, entry))])
    #directory = parent / "DATA_Maguire_20180517_ALL" / "W" / "TEST" / "UW"
    with alive_bar(fileCount) as bar:
...
def predictWebcam():
    ...
    while True:
        ...
        if out == "Crack":
            ...
            # Normalize image
            image_norm = cv2.normalize(image_conn, None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX,
                                       dtype=cv2.CV_8UC1)
